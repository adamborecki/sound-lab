...
- Day 5, EQ: Peak, Shelf & Notch — Parametric EQ (Peak / Bell), Low Shelf, High Shelf, Notch Filter, Filter Resonance, EQ: Peak, Shelf & Notch
...
Day 5 opens with EQ: unlike Day 4's Low-Pass/High-Pass/Band-Pass, which draw a hard line and remove everything past it, `peaking`/`lowshelf`/`highshelf`/`notch` are natively-implemented `BiquadFilterNode` types with a built-in gain param — boost or cut a region instead of cutting it outright. That also means none of Day 4's cascade-and-compensate machinery applies: a single node's Q and gain already match the number a student sets, since there's no 4-stage chain compounding them (see the comment atop `stations/eq-intro.js`). The four deep-dives (Parametric EQ, Low Shelf, High Shelf, Notch) come first, each with the same `musical loop / sawtooth tone / white noise` source picker as Day 4's filter stations; EQ: Peak, Shelf & Notch — the overview station mirroring Filters: Subtractive Synthesis's shape (a type selector over white noise, swapping which controls are visible — Gain for the three boost/cut types, Width (Q) for Peak and Notch, neither for the shelves) — comes last, as a capstone once the four shapes are already familiar rather than an introduction to them. Notch reuses Band-Pass's "Bandwidth in Hz" framing rather than exposing raw Q, converting with the plain `center / bandwidth` formula — no `bandpassQ()`-style compensation needed since a single notch node isn't cascaded. A real gap surfaced once the flashcard worksheet for this section tried to send students to a hands-on Q/resonance control on a Low-Pass or High-Pass filter: Day 4's `filter-lowpass.js`/`filter-highpass.js` are deliberately cutoff-only (see `filter-chain.js`'s fixed `STAGE_Q`), so there was nothing to point at. Filter Resonance fills it — a single (uncascaded) `lowpass`/`highpass` `BiquadFilterNode` with both Cutoff and Q exposed, so raising Q visibly grows a resonant bump right at the cutoff on the Filter Response curve before the roll-off, the same "cutoff + resonance" pair a synth filter section usually has. It widens `drawFilterCurve`'s vertical range to ±30 dB (options `minDb`/`maxDb`) since a high-Q peak can run well past a boost/cut EQ's usual ±18 dB. Vowels as Frequency follows as its own section: a simplified single-formant demo (a real vowel is shaped by two or three resonances at once) that runs one strong peaking boost over a student-selected source (buzzy sawtooth voice, white noise, or the musical loop) and lets a student sweep it through five preset frequencies (250 Hz-4 kHz, roughly "oo" through "ee") to hear the source turn vowel-like — proof that vowel identity rides on where a spectral peak sits, not on pitch.

All five EQ stations (the four deep-dives plus the overview) also draw a live **Filter Response** curve — gain in dB against log frequency, in the same visual language as a real plugin's EQ display. Unlike the Spectrum/Spectrogram views (which show whatever happens to be playing, indirectly implying the filter's shape through its effect on that material — hard to read for a boost/cut filter against noise), this reads the filter's own `getFrequencyResponse()` every frame: the browser's exact transfer-function math for that exact `BiquadFilterNode`, not an approximation, so the curve is unambiguously "this is what the filter is doing right now" and redraws live as a slider moves. `drawFilterCurve()` in `js/visualizers.js` is the shared implementation — log-spaced frequency samples (so it can share an axis-label row with `drawSpectrum`, via the same `logPositionForFreq` fractions), a dashed 0 dB reference line, and a filled deviation from it.
...
js/
  ...
  visualizers.js        waveform / spectrum / spectrogram / filter-response canvas rendering
...
stations/
  ...
  filter-resonance.js      Q/resonance on Low-Pass & High-Pass — Day 4's missing control, Filter Response curve
  eq-intro.js              EQ: Peak, Shelf & Notch capstone — type selector, conditional Gain/Q controls
  vowel-formants.js        single-formant sweep, 5 vowel presets, 3 sources (voice/noise/loop)
