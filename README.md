...
3. Use `audioEngine.createVoice(...)` for oscillators and `drawWaveform(canvas, audioEngine.analyser, ...)` from [`js/visualizers.js`](js/visualizers.js) for the live waveform. Both handle click-free fades and animation-loop cleanup for you. `drawSpectrum` and `drawSpectrogram` are the frequency-domain equivalents; `logPositionForFreq` keeps a station's own axis labels aligned with either.
...
stations/
  ...
  phase-polarity.js    continuous phase slider, A/B/Sum waveforms
