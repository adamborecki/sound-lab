...
export function mount(container, { audioEngine, accent }) {
  container.innerHTML = `
    ...
    <div class="osc-control" id="eq-gain-control">
      <div class="osc-control-label">Gain</div>
      <div class="big-readout" id="eq-gain-readout">${formatDb(DEFAULT_GAIN)}</div>
      <input type="range" id="eq-gain-slider" class="big-slider" min="${MIN_GAIN}" max="${MAX_GAIN}"
        value="${DEFAULT_GAIN}" step="0.5" aria-label="Boost or cut in decibels" />
    </div>

    <div class="osc-control" id="eq-q-control">
      <div class="osc-control-label">Width (Q)</div>
      <div class="big-readout" id="eq-q-readout">${DEFAULT_Q.toFixed(1)}</div>
      <input type="range" id="eq-q-slider" class="big-slider" min="${MIN_Q}" max="${MAX_Q}"
