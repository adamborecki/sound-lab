...
const STATION_ID = "filter-highshelf";
const MIN_CORNER = 500;
const MAX_CORNER = 10000;
const DEFAULT_CORNER = 3000;
const MIN_GAIN = -18;
const MAX_GAIN = 18;
...
export function mount(container, { audioEngine, accent }) {
  container.innerHTML = `
    <p class="prompt">
      A high shelf boosts or cuts <strong>everything above</strong> a corner frequency by one flat
      amount — a "treble knob." Below the corner, the signal is untouched. Unlike High-Pass, nothing
      gets removed, just turned up or down.
    </p>

    <div class="filter-shape-badge">${waveIconSvg("highshelf")}<span>High Shelf Response</span></div>

    <div class="preset-row" id="hs-sources"></div>

    <div class="osc-control">
      <div class="osc-control-label">Corner Frequency</div>
      <div class="big-readout" id="hs-corner-readout">${formatHz(DEFAULT_CORNER)}</div>
      <input type="range" id="hs-corner-slider" class="big-slider" min="${MIN_CORNER}" max="${MAX_CORNER}"
        value="${DEFAULT_CORNER}" step="1" aria-label="High shelf corner frequency in Hertz" />
    </div>

    <div class="osc-control">
      <div class="osc-control-label">Gain</div>
      <div class="big-readout" id="hs-gain-readout">${formatDb(DEFAULT_GAIN)}</div>
      <input type="range" id="hs-gain-slider" class="big-slider" min="${MIN_GAIN}" max="${MAX_GAIN}"
        value="${DEFAULT_GAIN}" step="0.5" aria-label="Boost or cut in decibels" />
    </div>

    <div class="osc-control-label">Spectrum (frequency)</div>
    <canvas class="spectrum-canvas" id="hs-spectrum-canvas" width="600" height="200"
      role="img" aria-label="Live frequency spectrum after the filter"></canvas>
    <div class="spectrum-axis" id="hs-spectrum-axis"></div>

    <div class="osc-control-label">Spectrogram (frequency vs. time)</div>
    <div class="spectrogram-row">
      <div class="spectrogram-freq-axis" id="hs-spectrogram-axis"></div>
      <canvas class="spectrum-canvas" id="hs-spectrogram-canvas" width="600" height="220"
  ...
  const sourceRow = container.querySelector("#hs-sources");
  const cornerSlider = container.querySelector("#hs-corner-slider");
  const cornerReadout = container.querySelector("#hs-corner-readout");
  const gainSlider = container.querySelector("#hs-gain-slider");
  const gainReadout = container.querySelector("#hs-gain-readout");
  const spectrumCanvas = container.querySelector("#hs-spectrum-canvas");
  const spectrumAxisEl = container.querySelector("#hs-spectrum-axis");
  const spectrogramCanvas = container.querySelector("#hs-spectrogram-canvas");
  const spectrogramAxisEl = container.querySelector("#hs-spectrogram-axis");
